...
class Grenzpolizei:
    def __init__(self, bot):
        ...
        self.event_types['on_message'] = 'message_event_channel'  #Added!
    ...
    async def _setup_questions(self, context):
        ...
        if server.id in self.settings:
            ...
            # Message events
            events['on_message'] = await self._yes_no('Do you want to track message attachments being sent? [y]es/[n]o', author) #added!
            ...
            if any([events['on_message'], events['on_message_delete'], events['on_message_edit']]):
    ...
    async def on_message(self, message):
        ...
        if await self._ignore(server, member=member, channel=channel):
            if await self._validate_event(server) and member.id != self.bot.user.id:
                embed = discord.Embed(color=self.red)
                avatar = member.avatar_url if member.avatar else member.default_avatar_url
                embed.set_author(name='Attachment Sent', icon_url=avatar)
                ...
                if message.attachments:
                    ...
                    for attachment in message.attachments:
                        ...
                        embed.set_image(url='{url}'.format(**attachment))
                    await self._send_message_to_channel(server, embed=embed)
                else:
                    return
